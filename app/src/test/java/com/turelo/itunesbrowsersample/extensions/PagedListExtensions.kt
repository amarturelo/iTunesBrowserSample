...
private val mockQuery = Mockito.mock<RoomSQLiteQuery>(RoomSQLiteQuery::class.java).also {
    Mockito.`when`(it.sql).then { return@then "" }
}

private val mockDb = Mockito.mock<RoomDatabase>(RoomDatabase::class.java).also {
    Mockito.`when`(it.invalidationTracker).then {
        return@then Mockito.mock(InvalidationTracker::class.java)
    }
}
